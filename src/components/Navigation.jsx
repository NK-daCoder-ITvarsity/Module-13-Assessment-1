              ...
              return (
                <li key={`primary-button-${index}`}>
                  <button
                    ...
                    className={`flex gap-1 items-center px-3 w-full text-sm border-r py-1 
                      ${isActive ? `${currentTheme.text} ${currentTheme.border} border-r-2` : 'text-stone-500'}
                    `}
                  >
                    <span
          ...
          {softwarePaths.secondaryPath.map((item, index) => {
            ...
            return (
              <li key={`secondary-button-${index}`}>
                <button
                  onClick={() => setUpdatedSection(item.label)}
                  className={`border-r flex gap-1 items-center px-3 py-1 w-full text-sm 
                    ${isActive ? `${currentTheme.text} ${currentTheme.border} border-r-2` : 'text-stone-500'}
                  `}
                >
                  <span
